...
function ClipResults({ clips, highlights }) {
  ...
  const loadVideo = async (url) => {
    try {
      console.log('Loading video from URL:', url); // Debug log
  ...
  const handleHighlightsDownload = async () => {
    try {
      console.log('Downloading highlights from:', highlights); // Debug log
      ...
      {highlights && (
        <div className="mb-6 p-4 border border-indigo-200 rounded-lg bg-indigo-50">
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-600">Combined highlights from most viral moments</p>
            </div>
