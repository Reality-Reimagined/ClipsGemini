...
def create_highlights_reel(video_folder: str, clips: list, min_viral_potential: int = 7, max_retries: int = 3):
    ...
    try:
        ...
        # First pass: Re-encode all clips with AMD acceleration
        for i, clip in enumerate(highlight_clips):
            try:
                ...
                # Try AMD GPU encoding first
                try:
                    ffmpeg.input(clip['url']) \
                          .output(reencoded_path,
                                 vcodec='h264_amf',        # AMD GPU encoder
                                 acodec='aac',
                                 video_bitrate='5000k',    # Higher bitrate for quality
                                 audio_bitrate='128k',
                                 **{
                                     'quality': 'quality',  # Focus on quality
                                     'profile:v': 'high',   # High profile
                                     'level': '4.1',        # Compatibility level
                                     'maxrate': '7000k',    # Max bitrate
                                     'bufsize': '5000k',    # Buffer size
                                 }) \
                          .overwrite_output() \
                          .run(capture_stdout=True, capture_stderr=True)
                    logging.info(f"Successfully re-encoded clip {i+1} with AMD acceleration")
                
                except ffmpeg.Error:
                    logging.warning(f"AMD encoding failed for clip {i+1}, falling back to CPU")
                    ...
                    ffmpeg.input(clip['url']) \
                          ...
                          .overwrite_output() \
                          .run(capture_stdout=True, capture_stderr=True)
                    logging.info(f"Successfully re-encoded clip {i+1} with CPU")
        ...
        logging.info(f"Content of concat file:")
        with open(concat_file, 'r') as f:
            logging.info(f.read())  # Log the actual content for debugging
        
        # Add retry logic for concatenation
        for attempt in range(max_retries):
            try:
                abs_highlights_path = os.path.abspath(highlights_path)
                
                logging.info(f"Concatenation attempt {attempt + 1}/{max_retries}")
                
                # Try concatenation with different methods based on attempt
                if attempt == 0:
                    # First try: AMD GPU concatenation
                    ffmpeg.input(concat_file, format='concat', safe=0) \
                          .output(abs_highlights_path,
                                 vcodec='h264_amf',
                                 acodec='aac',
                                 video_bitrate='5000k',
                                 audio_bitrate='128k') \
                    ...
                    try:
                        ...
                        logging.error(f"FFmpeg stderr: {e.stderr.decode() if e.stderr else 'No stderr'}")
                        logging.error(f"FFmpeg stdout: {e.stdout.decode() if e.stdout else 'No stdout'}")
                        raise
                else:
                    # Last try: CPU encoding
                    ffmpeg.input(concat_file, format='concat', safe=0) \
                          .output(abs_highlights_path,
                                 vcodec='libx264',
                                 acodec='aac',
                ...
                # If we get here, concatenation was successful
                logging.info(f"Successfully created highlights reel on attempt {attempt + 1}: {highlights_path}")
                break
                
            except Exception as e:
                logging.error(f"Concatenation attempt {attempt + 1} failed: {str(e)}")
                if attempt == max_retries - 1:  # Last attempt failed
                    logging.error("All concatenation attempts failed")
                    return None
                logging.info(f"Retrying concatenation...")
        ...
        # Only cleanup if highlights were successfully created
        if os.path.exists(highlights_path) and os.path.getsize(highlights_path) > 0:
            try:
                if 'concat_file' in locals() and os.path.exists(concat_file):
                    os.remove(concat_file)
                if 'reencoded_clips' in locals():
                    for clip_path in reencoded_clips:
